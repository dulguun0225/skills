---
name: backend-stack
description: How to choose a backend stack for an agent-written repo. ALWAYS load before picking a language, runtime, web framework, persistence library or database for a new backend, arguing that an existing stack should change, adding a second language to a repository or an organisation, or writing a rule set for a stack nobody has justified in writing.
---
