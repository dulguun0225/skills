...
2. **The two wireable checks ship as description, not as code.** **Narrowed 2026-08-02: both are now built and run over this skill set** — `npm run gates`, in the repository these skills distribute from (`https://github.com/dulguun0225/skills`, under `scripts/`), each exiting non-zero and printing what it not decide. What stay a gap: this skill carry no script a reader install, so reader still write or port one; the earlier implementation the source material record — one script in that project's own build — is still unpublished; and **nothing run the built pair automatically**, that repository have no CI, so it is a command someone type, weaker thing than a gate. **What building them produced is a finding about the rule, not about the files.** Measured 2026-08-02, re-runnable: stated as *every subheading in the evidence file name a real section of the directive text*, the evidence-order rule hold over 9 of 17 evidence files in that set and over none of the other 8 — those 8 organised by research pass, which is how the research happen. Gate ship with the 8 declared by name and reason, not with rule bent to fit.
