...
description: Java build checks for the async-handoff rules, plus the broker and outbox-library picks. ALWAYS load in a Java repo alongside async-handoff, before publishing a message, adding a queue or broker client, an in-process event bus, an executor submit, an @Async or @Scheduled task, a polled table or an outbound webhook.
---
