...
**2026-09-17, first feature built end to end (product-catalog 003-product-version).** Seven launches: three ended `needs-human` at review-spec, one at review-plan, one at analyze, each at the round cap with one or two fresh blocking findings, most of them holes opened by the previous round's fix; each time one fix agent applied the open findings and the run restarted at the next stage. Implement ran ten phases, every wall green, 103 tasks; converge reported converged in one round with zero findings. About 8.2M subagent tokens, of which roughly 5M were review, analysis and fix rounds before any code. Defect found: a run started after preflight had `state.wall = null`, every prompt said run `null`, and finish reported `wallGreen: false` on a green branch; the script now refuses such a start without `args.wall`. Open question, not decided: a review round with zero blocking findings could apply its majors once and proceed, as it does for minors.
