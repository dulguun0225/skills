---
name: money-storage
description: Rules for an amount of money that crosses a store boundary. ALWAYS load before adding or changing a money column, a migration, a view, a query that reads or totals an amount, a row mapper, or a stored balance.
---
...
- **Integer number of minor units as storage type** — `bigint` of cents, **and the measured bare default at the implementation tier** (probed 2026-08-11, bare sonnet at effort high: 2/2, exponent pushed onto every reader — the cost that got minor units rejected on wire at `M-12`). **Excluded by `M-10` wording, never justified on evidence**: no evidence survived on which decimal precision to pick, and argument on its side real — integer column cannot be over-scale, remove `M-30` whole failure mode by construction. Marked **convention**, **reopens** where language decimal support weak enough that `M-1` easier to enforce over integer type.
...
*`M-10` schema lint, extended to fail on money column declared with no precision and scale. Primary-source verified 2026-07-29 (PostgreSQL). Probed 2026-08-11 (bare sonnet, effort high): **binds, 0/2** — no session wrote a decimal column; both reached for `bigint` minor units (see the defaults list). Planned trim cancelled.*
...
*Schema lint over committed migrations. Convention, 2026-07-29, and **not premise-derived**: absent reader change stakes only through invention on read path, so this close to ordinary schema hygiene. Kept: cheap, fail toward safety. Probed pro-default at N=2 (bare sonnet, effort high, 2026-08-11); held by the lint.*
