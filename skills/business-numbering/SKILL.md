...
description: How a system issues human-facing numbers (account, loan, voucher, document). ALWAYS load before adding a number a person will read out, quote or type, writing an issuer, counter, format or check digit, adding any entry point that accepts a number, including a file import, writing a report that filters, groups or sorts on one, importing legacy numbers, or making a number gapless.
---
