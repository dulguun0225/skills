...
description: Rules for an agent-maintained repo. ALWAYS load before drawing or moving a module boundary, choosing a runtime topology, relaxing a build gate, adopting a framework, database, managed service or vendor API, writing a retry, error handler, repo constitution or code subtle enough to need a safety argument, adding a second way to do what the repo already does, or migrating off an existing system.
---
