---
name: primary-keys
description: Row identity, and what an id may never be used for. ALWAYS load before creating a table, choosing or changing a primary key, generating an id in application code, designing a human-facing number format, putting an id in a URL, payload, export, log line, log field set or object-storage key template, writing an ORDER BY over an id column in any language, or moving tenant data between databases.
---
