...
| **A bare-fixture firing case for the scaffolding moment** | `java-backend-rules` gained a trigger on 2026-09-01 — *before creating a build file or pinning the Java or Spring Boot version for a new repo* — because an agent scaffolding a greenfield backend pinned a superseded LTS. Both existing cases for that skill use an existing-repo fixture, which is exactly the hole; the description edit is unmeasured. **Since 2026-09-16 the scaffolding moment has a second answer**: `java-backend-template` pre-wires every gate, and the skill's body now says to start from it — so the case to write is no longer *does the skill fire on a bare fixture* alone but *does it fire on a fixture that is the template*, where the build file already exists and the trigger's premise is gone. The body edit is a pointer inside the SKILL.md, not a description change, so the sealed baseline still applies; whether the template pre-empts the load is the unmeasured half. **Since later on 2026-09-16 there is a second skill to point the bare case at**: `new-java-backend` owns the scaffold procedure and is meant to be invoked by name, so the question for it is whether "create a new Java backend here" earns the load without the name | One case, `"fixture": "bare"`; ≈ $0.50 a session at the sealed rate |
...
| Owed | Why it is owed | Watch |
| ---- | -------------- | ----- |
| **The review and analyze loops' stop rule — the owner's decision** | Nine stops ended at a round cap on findings a session then applied unchanged, or once applied none of: `wf_543886c7-762`, `wf_9cb9df52-800`, `wf_327e7ce1-c3a`, `wf_e6f0fb71-2ab`, `wf_9ecfdaff-a93`, `wf_510277de-2c6`, `wf_11cbb069-ff1`, `wf_85172f44-a39`, `wf_c3abfa9f-b08` — five features, three repositories. The loop has no fixed point, so a fourth round finds new findings as surely as a first. The candidate is converge's shape: stop on a **survivor**, and on a last round of new findings apply them and go on. **Two cap stops show the survivor half must stay** — `wf_c65ab41b-8a6` (a lock protocol refuted three rounds running) and `wf_e5ab8e1a-c53` (a declared narrowing of a requirement) each needed the owner's decision. The owner's 2026-09-18 audit put *apply the findings and move on* in his hands, which is why this is his and not an edit | Review findings carry no identity across rounds yet; converge's `repeatOf` is the precedent. Until 2026-09-24 a `from: "review-plan"` restart ran no review, so no fifth review has ever run — the counterfactual is still untaken |
| **The restart point after the session's own spec edit** | `build-feature` rejects re-running plan because it regenerates `plan.md` whole and discards every review fix, and then names `from: "plan"` as the restart after a spec change. On eight such restarts the regenerated plan hit a fresh review-plan cap five times (`wf_c65ab41b-8a6`, `wf_9ecfdaff-a93`, `wf_11cbb069-ff1`, `wf_85172f44-a39`, `wf_c3abfa9f-b08`), which is what held `reference-data` 001 at F3's fixed point. `from: "review-plan"` re-reads the existing plan against the edited spec and repairs it in place — possible only since the 2026-09-24 fix that makes that start run the review | Unrun in that form: nothing has restarted at review-plan with a working review. Touches the spec section and resolution rule of `build-feature/SKILL.md`, the handoff's restart sentence and the two `specChanges` exits in `workflow.mjs`, and the preflight exit for a spec that moved under a later start |
| **Spec readiness before sign-off** | Eight stops whose cause was already in the spec text before the run: a bound or fact the spec deferred to the plan, a criterion with no measurement conditions, unstated search semantics, a reversed clarify answer's residue, two requirements that disagree — `wf_de4bfd27-d8a`, `wf_bc7b0f7c-ce6`, `wf_fbb2141f-8bf`, `wf_115f053c-900`, `wf_e4e27b83-bc3`, `wf_6e72f3dd-930`, `wf_a6f3b709-43a`, `wf_e68e4e48-4ed`. The earliest stage is the author's clarify, so the prevention is on the consumer side: a read-only refutation of `spec.md` against itself, the constitution and the sibling specs that returns questions and never edits. **Where it runs is the owner's call** — stock spec-kit, a new skill (frontmatter paid every session), or `build-feature`'s preflight, which moves the stop before plan rather than preventing it. Beside it, a smaller question: whether a MEDIUM or LOW spec finding should stop a run at all | One such pass is the old review-spec stage without its write access; it was removed 2026-09-21 for writing the spec, not for reading it |
| **The handoff's journal line** | Step 2 of the handoff prompt takes the newest `wf_*.json` on disk, and the running journal is, by inference, not there yet: 25 of 27 handoffs name another run's journal or none (`d364c61` on `product-catalog` hand-corrected one on 2026-09-18). A durable file states a false path | `workflow.mjs` handoff prompt, and the sentence of `build-feature/SKILL.md` that says the file carries the journal's path. The session that launched the run has the run id on its tool result |
| **Whether an owner's recorded deferral exempts a gap from forcing** | `wf_e4af9888-80a` forced and built 003/FR-034, which the owner's post-plan clarification had deferred; he kept it. The floor-`NONE` rule of 2026-09-20 says it should be built; whether a deferral the owner wrote into the spec overrides that is his question, and nothing here decides it | One run. Sweep 1's named-gap recurrence is decided *nothing* in Sweep 2 |
...
## Evidence owed on a published skill — deferred, 2026-08-03

**Owner decision, 2026-08-03: this whole section is far-future work.** It is
not withdrawn and nothing in it is answered — the five conversion skills still
ship everything at *convention* with a central claim at *uncertain*, and the
lapse rule still runs against them on schedule. What changed is priority: none
of it is scheduled, and no other row on this page waits on it. Read the rows
below as the standing description of what the set owes, not as queued work.
