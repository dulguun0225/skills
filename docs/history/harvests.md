...
**That row is gone too: `business-numbering` published 2026-08-02**, and it is the
first row here whose source was already read cover to cover before the harvest
started.

**What the fourth one changed about the cost estimate: it is the first row that
named a real location and the estimate still held.** Every earlier row named
"research notes" that turned out not to exist; this one named a decision record and
a use case, both of which were where it said. **The cost went somewhere else
instead** — into deciding what a *neutral* skill may carry when its source is
saturated with one jurisdiction's banking rail, and into keeping the sibling split
honest, since `primary-keys` had already published part of this material
and each piece had to be pointed at rather than restated — **a claim that was itself
wrong, and is corrected in the note at the top of this file.** **The reading window records
what was not opened**, and two of those could still change a directive: the
seed-pack specification, and the per-domain use cases where each class's issue
moment is actually fixed.
