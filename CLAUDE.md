...
Skills repo: publish dated, researched engineering-decision rules as Agent Skills, install with Vercel `skills` CLI (skills.sh), **not listed in public skills.sh directory**.

State 2026-08-01: no build/lint/test tooling beyond discovery check. Exist: project setup (node + `skills` CLI pinned, LF repo-wide, commands as npm scripts — see *Commands*), decided skill decomposition + layout (*Where skills live*), and **every row of table in *Where skills live* authored — Milestone 1 done. All three cross-stack sources converted, both *packs* converted, both corpus-derived method skills written**:
...
- **`guardrails-toolchain`, authored 2026-08-01** — `skills/guardrails-toolchain/` (`SKILL.md` + `evidence.md`), 14 directives, **no rule ids**. **First skill harvested from `BACKLOG.md`**, second not drawn from corpus. See *The `guardrails-toolchain` skill*.

- **`ai-maintainer-principles`, authored 2026-08-01** — `skills/ai-maintainer-principles/` (`SKILL.md` + `evidence.md`), 12 directives, **no rule ids**. Second harvested from `BACKLOG.md`, third not drawn from corpus. **First whose named source no exist** — row named research notes nobody hold, topic assembled across that pass's decision records instead. See *The `ai-maintainer-principles` skill*.

- **`primary-keys`, authored 2026-08-01** — `skills/primary-keys/` (`SKILL.md` + `evidence.md`), 15 directives, **no rule ids**. Third harvested from `BACKLOG.md`, fourth not drawn from corpus. **First whose source record carry an adversarial structure in own status line** — three-way evaluation with grounds per loser — and **first harvest where reading a neighbour changed a directive rather than decorating it.** See *The `primary-keys` skill*.

Every one discovered by `npm run check` — **compare its output against `ls skills/`, never against a number in this file.** **Milestone 1 — turn imported corpus into skills — done**: three cross-stack sources, both packs, both corpus-derived rows converted; no row of table in *Where skills live* unwritten. **What corpus hold no skill carry listed in *What becomes no skill*** under that table — bookkeeping + consuming-repo machinery by construction, not by omission.
...
Deletion also **close last decision Milestone 1 left open** — two sources instantiation tables want sentence saying skills row absent on purpose, and writing it would have been authoring in imported material, which old rule forbid. No file, no conflict.

**Cost of deletion, stated as choice not surprise.** Line-range provenance gone, so every adversarial review recorded below no re-runnable against source; token-placement check (extract identifier tokens from source region, require each per directive) have no input — it found twelve hedged tool names in Java-backend family, twenty-three more on second pass; corpus own instantiation tables + audits-owed backlog gone, latter salvaged into `BACKLOG.md` re-keyed to skills. All three recoverable at sha above. None reach published skill.
...
Two gates deleted corpus refer to **never in this repo**: `ci/check_packs.py` (fail build on mis-grouped evidence subheadings and on rule ids or links in paste text) and `bundle-checks.yml` freshness step (warn when `review-by` passed). `enforceable-rules` tell reader wire equivalents. Want them here → must write them.
...
| Skill | Drawn from |
| ----- | ---------- |
| `java-backend-rules`, `java-backend-api`, `java-backend-observability` | Java-backend pack paste text minus its money, cache and broker sections, already converted; evidence + dates from that pack evidence notes under same eight `###` headings, plus its rejected-alternatives + re-open-trigger sections. Split in *The Java-backend skill family* |
| `llm-default-traps` | agent-traps pack paste text; evidence from that pack evidence notes |
| `money`, `money-api`, `money-storage`, `money-java` | money-grade source + money section of Java-backend paste text — per-skill split + evidence map in *The money skill family* |
| `caching`, `caching-java` | cache-discipline source + cache section of Java-backend paste text; Java evidence from that pack evidence notes under `Cache discipline`. Split in *The caching skill family* |
| `async-handoff`, `async-handoff-shapes`, `async-handoff-java` | event-broker-discipline source + broker section of Java-backend paste text; Java evidence from that pack evidence notes under `Event broker discipline`. Split in *The asynchronous-handoff skill family* |
| `tech-decision-research` | research protocol, its framing / panel / refutation / dating / re-verification sections. Its confidence markers land here; corpus README duplicate definition of same four not carried twice. Split in *The method skills* |
| `enforceable-rules` | corpus README design principles + premise-specificity test; portable ship checks from research protocol; plus that README *Markers* — **enforcement** markers + **status tier** only — and its Anatomy tripwire, which table no assign. Split in *The method skills* |
...
2. **Shipped skills cite `M-n`.** See narrowed invariant under *Authoring invariants*.
...
**Rewritten wholesale, not carried verbatim.** No skill file hold byte-identical copy of any corpus text, so diff gate in *Where skills live* no apply to phase 1, nothing to diff. Reason: half-copy never available — the money-grade source directives directive text carry corpus vocabulary meaning nothing to consumer ("a stack pack states it once"), and each skill add premise, rejected default, instructions to agent around every directive.
...
- **`C-16` = `M-29` shape.** In corpus it arm tripwire; skill not pasted, so **description** = tripwire and `C-16` ship as obligation to record decision in plan.
...
1. **One neutral skill, not three.** Money cut three neutral skills on extra conditions its source state. The cache-discipline source state one extra condition — repo cache value it could recompute — plus `C-9` in-process-and-multi-instance carve-out; neither decomposition axis. 16 directives fit one `SKILL.md`.
2. **Engine pick = directive in `caching-java` with no `C-n` id, and stack skill say so on first screen.** One place a `caching-java` entry not keyed to neutral id, no oversight: the cache-discipline source when-this-applies section deliberately keep engine pick out of source cuz its gates no vary by stack, its right answer vary *within* stack, and it fail premise-specificity test. All three grounds ship, in `caching/evidence.md`. **Contrast `money-java`, where every entry have id** — reader assuming that invariant hold everywhere would hunt for missing directive.
...
- **Same as after money family: no check enforce any conversion invariant.** Id-uniqueness, no-`P-n`, no-`B-n`, no-link-out-of-dir, no-corpus-reference sweep over these three files run by hand 2026-07-30, clean. `npm run check` see neither `evidence.md`.

### The adversarial review, 2026-07-30

All three files re-read against the cache-discipline source, the Java-backend paste text cache section, the Java-backend pack evidence notes under `Cache discipline`, plus four published money skills. **Structure held**: 16 directives defined exactly once, no `P-n`, no `B-n`, no link out of skill dir, no reference into corpus, every directive carry kind + marker + date, all six skills listed by `npm run check`. Every content defect found fixed, in four files — `caching/SKILL.md`, `caching/evidence.md`, `caching-java/SKILL.md`, **`money/SKILL.md`**, one nobody expected to touch. **Four findings generalise:**
...
**Two corpus-derived conversions, last rows of table in *Where skills live*.** Both listed by `npm run check`. Drawn from the research protocol framing, panel, refutation, dating and re-verification sections (`tech-decision-research`) and from the corpus README *Markers*, *Authoring a pack*, one Anatomy item plus the research protocol ship checks (`enforceable-rules`).
...
Seventeenth skill, listed by `npm run check`. **No row in *Where skills live***,
...
Eighteenth skill, listed by `npm run check`. **No row in *Where skills live*** — that table = corpus provenance, this have none. Drawn from sibling repo `../net-saas`: **the architecture decision records** of same 2026-06-11..14 pass that ground `backend-stack` + `guardrails-toolchain`, plus one non-authoritative counterfactual variant and one note written for management.
...
Rules every skill here held to. Came from deleted corpus; **published skills = authority now** — `tech-decision-research` define confidence markers + method, `enforceable-rules` define enforcement markers, status tier, premise-specificity test, design principles, and the predicate, composite-shape, layer, enumeration and token-placement checks. Read those before authoring, not this summary.

- **Rule ship with named check + enforcement marker, or not rule.** Never restate directive without parenthesised check.
...
- **Directive text + evidence stay separate.** `SKILL.md` = instinct-override payload for scarce context window; `evidence.md` = for human deciding whether to trust it. Progressive disclosure map onto this directly, and it why no family fit one always-loaded body.
- **Ids resolve inside installed skill dir, or no ship.** `M-n`, `C-n`, `E-n` ship cuz consumer installing `money-java` have `money/`, `money-api/`, `money-storage/` on same disk. `P-n` **never** ship, and rule absolute not contingent: `enforceable-rules` publish those eight principles so id would now resolve for whoever install it — still out, cuz other skills refer to them in prose, so id resolve for one reader and dangle for other, and stable `###` heading name carry whole point of never-renumbering (*The method skills*, decision 4). `DECISIONS.md` `B-n` unreachable and never appear. No relative link leave own skill dir.
- **Name corpus favourite and why it lost.** "Use X" no override agent instinct; "the default is Y, rejected because Z" do. That sentence most important line in skill — no compress it away.
- **Where duplication deliberate, one owner + one index** — only thing that catch drift. Preferred fix = write-once, each directive text exist exactly once, which every family did.
- Directive shape: **bold directive**, then reasoning, then check in parentheses with its enforcement marker.

**Markers, per claim and per rule.** Confidence: *confirmed* (survived three independent refutation votes against primary sources) / *primary-source verified* (one researcher, no panel) / *convention* / *uncertain*. Enforcement: *off-the-shelf* / *bespoke* / *convention*. Status tier: *production-confirmed* / *decided, not yet validated* (= researched + decided, **no production use yet**) / *deferred — evidence-driven*. **Lapse rule**: past `review-by`, every *confirmed* marker read as *convention* until new pass re-date it — no maintainer action needed.

**Shared premise** every skill state: code written by LLM agents and no human read it line by line. Rules conditioned on it; verdicts portable exactly as far as their premises.

**Two invariants narrowed by last conversion; narrowings ones to know** (2026-07-30, *The method skills*):

- **"Dates and markers travel with the claim" assume source have date.** Method-skill material carried no frontmatter, no marker, no date at all. Rule hold by being applied honestly not literally: **no date invented**, conversion date stated once and labelled as such, markers derived by applying downgrade rule to material — landing every directive in both skills at *convention*. **Invent per-directive date would be exact failure this invariant exist to prevent, in reverse.**
- **"A rule ships with its named check, or it is not a rule" about rules that bind code.** Both method skills directives bind *process*, and their check = written artifact whose absence visible. Contradiction stated inside skills not hidden by hedged tool names, and one place published skill declare principle no reach it.

Where skills live and what one skill is: *Where skills live*. Two questions about shape of directive in skill were open per skill, listed at end of that section. **Every authored skill set answered both and nothing left to raise either again** — see *The money skill family*, *The caching skill family*, *The asynchronous-handoff skill family*, *The `llm-default-traps` skill*, *The Java-backend skill family*, *The method skills*. Three cross-stack families answered identically. Both pack-derived sets answered second question same way and first differently, cuz neither neutral/stack pair: each name tool beside check kind in same file, `llm-default-traps` as one skill and Java-backend family as three cut by what agent doing. Two corpus-derived skills answered first in third way — **most of their directives can have no gate at all** — and second as everything else did.